...
int main(){
	...
	cin >> N.num;
	bool SCP = false;
	bigNum minX, maxX, mid, One;
	One.sign = minX.sign = maxX.sign = mid.sign = 1;
	minX.num = "0";
	One.num = "1";
	maxX.num = N.num;
	while(cmp(minX, maxX) <= 0){
		mid = bigDiv1(add(minX, maxX), 2);
		bigNum multi = mul2(mid, mid);
		if(cmp(multi, N) == 0){
			SCP = true;
			break;
		}
		
		if(cmp(multi, N) > 0) maxX = sub(mid, One);
		else minX = add(mid, One);
	}
	cout << (SCP ? "YES" : "NO");
}
/*
